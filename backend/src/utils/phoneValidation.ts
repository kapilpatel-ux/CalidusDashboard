...
export const phoneDigits = (value: unknown) =>
  String(value || "")
    .replace(/^00/, "+")
    .replace(/\D/g, "");

export const getPhoneRuleForCountry = (country: unknown) => {
  const normalized = normalizeCountry(country);
  if (!normalized) return null;
  return normalizedRules.find((rule) => rule.normalizedNames.includes(normalized)) || null;
};

const getRuleFromPhone = (digits: string) =>
  normalizedRules
    .slice()
    .sort((a, b) => b.dialCode.length - a.dialCode.length)
    .find((rule) => digits.startsWith(rule.dialCode));

const validateLength = (digits: string, min: number, max: number, label: string) => {
...
export const validatePhoneNumber = (value: unknown, country?: unknown) => {
