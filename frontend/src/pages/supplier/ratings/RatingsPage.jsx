...
import { currentSupplier, products as allProducts, ratings as allRatings } from "@/data/mockData";

export const SupplierRatings = () => {
  const supplierProducts = allProducts.filter((product) => product.supplierId === currentSupplier.id);
  const [ratings, setRatings] = useState(allRatings.filter((rating) => supplierProducts.some((product) => product.id === rating.productId)));
  ...
  const submitReply = () => {
    if (replyText.trim() && replyDialog.rating) {
      setRatings((prev) => prev.map((rating) =>
        rating.id === replyDialog.rating.id
          ? { ...rating, supplierReply: replyText, supplierReplyStatus: "pending" }
          : rating
      ));
    ...
    <>
      <div className="space-y-6" data-testid="supplier-ratings">
        ...
        <div className="space-y-4">
          {ratings.map((rating) => (
            <div key={rating.id} className="dashboard-card">
              <div className="dashboard-card-content">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="sm" className="gap-2" onClick={() => { setReplyText(""); setReplyDialog({ open: true, rating }); }}>
