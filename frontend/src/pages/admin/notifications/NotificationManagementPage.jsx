...
export const NotificationManagement = () => {
  ...
  const [updateRead] = useUpdateNotificationReadMutation();
  ...
  const handleRead = async (row, read) => {
    try {
      await updateRead({ id: row.id, read }).unwrap();
  ...
  const columns = [
    ...
    { key: "date", label: "Date & Time", render: (_, row) => formatNotificationDateTime(row) },
  ...
  return (
    <div className="space-y-6" data-testid="admin-notification-management">
